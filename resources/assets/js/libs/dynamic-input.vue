<template>
	<div class="dynamic-input-wrapper">
		<label class="control-label" v-show="input.label" :for="input.name" >{{input.label}}</label>
		
		<!-- start input type if -->
			
			<!-- hidden input -->
			<input v-if="input.type === 'hidden'" :class="input.class" type="hidden" :name="input.name" v-model="input.value" :value="(input.value)?input.value:input.default">
			...
			<!-- number input -->
			<input v-else-if="input.type === 'integer'" :class="input.class" type="number" :name="input.name" :min="input.min" :max="input.max" :step="input.step" v-model="input.value">

			<!-- textarea -->
			<textarea v-else-if="input.type === 'longtext'" :class="input.class" :name="input.name" v-model="input.value" :placeholder="input.placeholder"></textarea>
			...
			<div v-else-if="input.type === 'radio'" class="radio">
				<label class="radio-inline" v-for="option in input.options">
					<input type="radio" :name="input.name" v-model="input.value" :value="option.value" :id="input.name + '_' + option.name" >
					{{ option.name}}
				</label>
			</div>
		...
		<!-- error span -->
		<div v-show="error" class="help-block">
			<ul v-if="error instanceof Array" class="list-unstyled">
